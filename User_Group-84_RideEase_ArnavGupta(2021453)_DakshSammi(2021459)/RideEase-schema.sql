...
-- Retrieve the names and email addresses of all active drivers.
-- Relational algebra: π fname, lname, E_mail (σ Dr_Status='active'(Driver))
SELECT Driver_ID as DriverID,fname as DriverName, Driver_Rating as DriverRating, Dr_Status as DriverStatus,E_mail as DriverEmail
FROM Driver
WHERE Dr_Status = 'inactive'
Group by Driver_ID;



-- Retrieve the first name, last name, and email address of the driver with the highest rating.
-- Relational algebra: π fname, lname, E_mail (σ Driver_rating = MAX(Driver_rating)(Driver))
SELECT fname, lname, E_mail
FROM Driver
WHERE Driver_rating = (
...
-- Find the names and email addresses of all users who have not booked a ride.
-- Relational algebra: R1 ← π fname, lname, E_mail (User)
-- R2 ← π UserID (Bookings)
-- R3 ← R1 - (R1 ⨝ UserID=R2.UserID Bookings)
SELECT fname, lname,E_mail
FROM Users
...
-- Retrieve the names and email addresses of all drivers who have a rating greater than the average rating of all drivers.
-- Relational algebra:
-- π fname, lname, E_mail (σ Driver_rating > (AVG(Driver_rating)) (Driver))
SELECT fname, lname, E_mail
FROM Driver
WHERE Driver_rating > (SELECT AVG(Driver_rating) FROM Driver);


-- Retrieve the total number of rides and the total distance driven by each hired driver in the system.
-- Relational algebra:
-- π Driver_ID, SUM(Distance), COUNT(*) (DrivesFor)
SELECT DrivID AS Driver_ID, SUM(Distance) AS total_distance, COUNT(*) AS total_rides
FROM DrivesFor
GROUP BY DrivID;



-- Retrieve the top 3 most popular pickup locations.
-- Relational algebra:
-- γ Pickup(π Pickup, COUNT(*) → count(σ Pickup is not null (Bookings))) 
SELECT Pickup as Location, COUNT(*) AS Bookings
FROM Bookings
...
-- Problem Statement: Insert a new record for a user with the following details:
--     First name: John
--     Last name: Doe
--     Email: john.doe@example.com
--     Password: password123
--     Status: 'Normal'
-- Relational Algebra: Users ⨝ {('John',NULL,'Doe','Normal','john.doe@example.com','password123')} on {E_mail}
INSERT INTO Users (fname, mname, lname, User_Status, E_mail, UserPassword)
VALUES ('John', NULL, 'Doe', 'Normal', 'john.doe@example.com', 'password123');


-- Problem Statement: Display the email addresses and first name of all users whose first name starts with 'J'.
-- Relational Algebra: π(E_mail)(σ(fname LIKE 'J%')(Driver))
SELECT E_mail,fname FROM Driver
WHERE fname LIKE 'J%';

-- Find the total number of bookings made for each driver.
SELECT D.Driver_ID, COUNT(B.Booking_ID) AS Num_Bookings
FROM Driver D
LEFT JOIN Bookings B ON D.Driver_ID = B.Dri_ID
GROUP BY D.Driver_ID;

-- Find the vehicle information for all rides.
SELECT R.RideID, V.License_Platenumber, V.Model, V.Make
FROM Ride R
JOIN Vehicle V ON R.RideID = V.Vehicle_ID
GROUP BY R.RideID;


-- OLAP QUERIES

-- 1) Retrieve the total distance driven by drivers in the system for each month of the year and each year.
...
-- 2)Retrieve the fname and lname of users in the system for each month of the year and each year who have availed a ride.
...
-- 3)Retrieve the number of bookings made for each ride type and the average fare for each ride type.
...
-- 4)Retrieve the names of drivers in the system whose rating is greater than 3 grouped by every month of the year and every year.
...
-- 1)Trigger to update the driver status to "inactive" when they accept a ride request.
DELIMITER $$
CREATE TRIGGER Driver_Status_Trigger
...
INSERT INTO `Bookings` (`Booking_ID`, `UserID`, `Dri_ID`, `Pickup`, `Dropoff`) VALUES (101, 39, 13, 'Flat 72 96 Hermina Upper\nNew Alexandreside, ACT 24', '89 Ward Green\nNorth Beverly, ACT 8498');
select * from Driver;


-- 2)Trigger to update the customer status to Prime if the user has been part of more than 1 rides.
CREATE TRIGGER user_trigger
AFTER INSERT ON DrivesFor
FOR EACH ROW
BEGIN
...
INSERT INTO `DrivesFor` (`DrivID`, `UsrID`, `Tim`, `Distance`) VALUES (15, 3, '2021-12-12 11:12:18', '495');
Select * FROM Users;